...
var creatorList = ['@zyxmaple','@alpha'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/62895378264836 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'ZyxMaple') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'ZyxMaple') return res.json(loghandler.invalidKey)
